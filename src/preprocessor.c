...
void combine_modules(preproc_state* state)
{
    ...
    for (size_t i = modules_vec->size - 1; i != UINT64_MAX; i--)
    {
        // copy child nodes
        module* dependency = vector_get_item(modules_vec, i);
        for (size_t j = 0; j < dependency->ast_root->children->size; j++)
        {
        ...
        for (size_t j = 0; j < dependency->ast_root->value.symbol_table->capacity; j++)
        {
